...
function vote (uint _id, Option _option ) public onlyActiveMembers{
    ...
    require(members[msg.sender].isBlacklisted == false && votes[_id][msg.sender] == false, "only non blacklisted members who havne(t voted for the proposal _id can vote");
    if (Option.Yes == _option)  proposals[_id].counterForVotes++;
    else if (Option.No == _option) proposals[_id].counterAgainstVotes++;
    else if (Option.Blank == _option) proposals[_id].counterBlankVotes++;
